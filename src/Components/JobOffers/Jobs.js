...
const Jobs = (props) => {
  return (
    <div className={props.mobile?'text-white  flex-col bg-[#222222]  rounded-3xl box-border hidden lg:flex ml-0 py-4 w-5/6 max-w-[50px] lg:max-w-none lg:w-auto mb-8 lg:mb-0':' text-white flex  flex-col bg-[#222222]  rounded-3xl box-border ml-0 mb-8 lg:mb-0'}>
      <div className='flex items-center gap-6 mb-6 pt-6 xl:gap-6 xl:mb-6 xl:pt-6 md:gap-6 md:mb-4 md:pt-3'><div><img src={props.image} className = 'md:w-7 pl-4'/> </div><h3>{props.title}</h3></div>
      <div className="font-Montserrat-Bold mb-6 pl-6 xl:pl-6 xl:mb-6  md:pl-4 md:mb-4">{props.text}</div>
      <div className='flex gap-4 items-center pl-4 mb-4 xl:pl-4 xl:mb-4 md:pl-4 md:mb-3'><h3 className='flex gap-1'><span className='w-5'><img src={props.locmage} className = 'w-full'/></span><>{props.location}</></h3><h3 className='text-[#D23431]'>{props.mode}</h3></div>
